...
适用范围：智能底盘（25 槽，nRF52811 + NT3H2111）与 VibeRack APP（Android/iOS）之间的全部蓝牙交互。本文档是固件与 APP 的共同契约，任何修改需同步更新协议版本号。
...
| Device Health 服务 | 自定义 128-bit（基 UUID + 0x0003） | 电量、复位原因、健康标志 |
| Battery Service | 0x180F（标准，可选） | 电量百分比，notify |
| Device Information | 0x180A（标准，可选） | 固件版本、硬件版本、型号 |
| Secure DFU | Nordic 标准（可选） | 无按钮 OTA 升级入口 |

自定义基 UUID 定一个随机 128-bit，全产品线复用；开发期以 UUID 第一段低 16 位 short id 区分服务/特征。

Binding Table、Light、Device Health 为 v0.1 必备自定义服务。标准 BAS/DIS/DFU 是可选补充能力，APP 不得因 BAS 或 DIS 未发现而阻塞 Binding/Light/Device Health 闭环。
...
| Device Health Service | 0x0003 | `7f4b0003-8d1a-4d45-9a4e-2b4a7c000000` |
| Device Health characteristic | 0x3001 | `7f4b3001-8d1a-4d45-9a4e-2b4a7c000000` |
...
| 特征 | 属性 | 说明 |
|---|---|---|
| Device Health | Read / Notify | 4B 健康快照，见 7.2 |

### 7.2 Payload 格式（4 字节）

| 偏移 | 长度 | 字段 | 说明 |
|---|---|---|---|
| 0 | 1 B | battery_pct | 0-100，连接后的当前电量 |
| 1 | 2 B | reset_reason | uint16 小端，固件复位原因码 |
| 3 | 1 B | health_flags | bit0 低电量；bit1 flash busy/降级；bit2 传感器异常；bit3 灯控异常；其余保留 |

当前 XIAO 开发板样例 payload 为 `64 02 00 00`，表示 `battery_pct=100`、`reset_reason=0x0002`、`health_flags=0x00`。

广播中的 `battery_pct` 用于免连接列表渲染；Device Health 是连接后的健康事实源。若标准 Battery Service 或 Device Information 不存在，APP 仍应继续完成 Device Health、Binding Table 与 Light 服务初始化。
...
5. 固件断言/看门狗复位后首包 Device Health Notify 携带复位原因码，便于现场排查。
